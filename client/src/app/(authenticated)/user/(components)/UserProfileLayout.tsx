...
const UserProfileLayout = ({ session }) => {
    const { fetchUserInfoById, getAvatarList, updateAvatar, updateUsername } = useUser();
    const [username, setUsername] = useState('');

    const { data: userInfo, isLoading: isUserInfoLoading, refetch: refetchUserInfo } = useQuery({
        ...
        <>
            <div className="min-h-screen bg-gradient-to-b from-zinc-900 to-black p-4 md:p-8">
                <Card className="mx-auto max-w-7xl bg-zinc-900/50 border-zinc-800">
                    <div className="p-6">
                        <h1 className="text-2xl font-semibold text-white mb-8">Thông tin cá nhân</h1>
                            ...
                            <div className="flex-1 text-white">
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        ...
                                        <Input disabled id='phone' value={userInfo?.phone || "Không có dữ liệu"}></Input>
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="coin">Skycoin</Label>
                                        <Input disabled id='coin' value={userInfo?.coinPoint || "Không có dữ liệu"}></Input>
                                    </div>
                                    <div className="space-y-2">
